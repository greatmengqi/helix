...
object Agent {
  ...
  /** L5 meta-loop：在 [[loop]] 外层再套一圈，`judge` 决定本轮答案"够不够好"， 不够就把 `judge`
    * 返回的消息注入 history，重入下一轮 [[loop]]。
    *
    * **这不是新 effect，是把 [[loop]] 当 pure-ish 的 `(history) => (ans, history)` 变换反复迭代**——
    * 用 Kyo `Loop` 承载外层状态，内层 [[loop]] 的所有 L2/L3/L4 wire 原封照用（签名即证）。
    *
    * 对应能力：
    ...
    *   - `judge(ans, history)`：返回 `None` 终止并返回 `(ans, history)`； 返回 `Some(hint)` 把
    *     hint 追加到 history 末尾，remaining-1 后重入 [[loop]]
    *
    ...
    *
    * 边界约定（`maxRounds < 1`）：`Abort.fail(IllegalArgumentException)`——显式拒绝无意义输入，
    * 不 silent 退化为 1 轮。调用方要动态决定是否 replay 时自己判断。
    *
    * 判决函数纯 Scala，不取 effect——如果 judge 需要调 LLM / Tool，再加一个 overload 把
    * signature 升级为 `judge: ... => Option[Message] < (LLM & ...)`。当前 MVP 不做，等消费场景出现。
    *
          ...
          for {
            ...
            next <- judge(ans, updated) match {
              ...
              case Some(_) if remaining <= 1 =>
                ...
                Log
                  .info(s"replay.stop reason=budget-exhausted remaining=$remaining")
                  .map(_ =>
                    Loop
                      .done[List[Message], Int, (String, List[Message])](
                        (ans, updated)
  ...
  /** [[loopWithReplay]] 单发 overload：一次性提问场景，只关心终答不关心 history。 完全等价于用
    * `List(Message(Role.User, userInput))` 起一个 history 跑 stateful 版再 `.map(_._1)`。
    *
    * `innerMaxSteps = DefaultMaxSteps` 的默认值只在这个 overload 上——history 版本要求显式传，
    * 避开 Scala 3 overload + default accessor 命名冲突（决策 #3）。
    */
  def loopWithReplay(
      userInput: String,
      maxRounds: Int,
