...
/** L4 `ArrowEffect`：允许 middleware 在每步开始时请求早停。
  ...
  * 独立 effect，不合并到 god-ADT。加新能力（例如 `ResponseHook`）= 加新 sealed trait， 不扩已有的
  * `Option[String]` signal shape。
  *
...
object AgentHalt {
  ...
  /** **invoke**：raise 一次 halt check——每轮主流程入口调一次，handler 决定 `Some(reason)` 早停或
    * `None` 继续。
    *
    * **三层角色**（source-level）：
  ...
  /** **impl**（on）：条件早停——每次 invoke 调用时重新求值 `guard`，`Some(reason)` 立即终止、`None`
    * 继续。
    *
    * `guard: => Option[String]` 是 by-name——handler 每次拦截都重新求值，所以 guard 里读取的外部
    * state（`AtomicInteger.get()` / `AtomicBoolean.get()` / 时钟等）总是最新的。
    *
    * 不做"halted 后仍被调用"的二次保护——loop 在 `haltOpt = Some(...)` 分支就 `Loop.done`， 不会继续
    * invoke。
    */
